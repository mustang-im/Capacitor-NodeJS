...
# ----------------------------------------
# Required Capacitor environment variables
# ----------------------------------------
if [[ -z "$CAPACITOR_ROOT_DIR" ]]; then
  echo "Error: CAPACITOR_ROOT_DIR is not set."
  exit 1
fi

if [[ -z "$INIT_CWD" ]]; then
  echo "Error: INIT_CWD is not set (should be platformDir)."
  exit 1
fi

# Capacitor directories
PROJECT_ROOT="$CAPACITOR_ROOT_DIR"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PACKAGE_DIR="$(cd "$SCRIPT_DIR/../../" && pwd)"

FORCE=false
PLATFORM="$CAPACITOR_PLATFORM_NAME"

usage() {
  echo "Usage: $0 [--force <true|false>] [--platform <android|ios>]"
  echo "  --force      Force re-download/copy even if destination exists"
  echo "  --platform   Only process specified platform"
  exit 1
}

while [[ $# -gt 0 ]]; do
  echo "Processing argument: $1 with value: $2"
  case $1 in
    --force)
      FORCE=$([[ "$2" == "true" ]] && echo true || echo false)
      shift 2
      ;;
    --platform)
      PLATFORM="$2"
...
process_platform() {
  ...
  if [[ -z "$lib_path" ]]; then
    echo "[$platform] No $config_key found, using fallback"
  ...
  if [[ "$lib_path" == http* ]]; then
    download_and_extract "$lib_path" "$dest_dir"
  else
    [[ "$lib_path" != /* ]] && lib_path="$PROJECT_ROOT/$lib_path"
